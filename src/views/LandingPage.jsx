...
const LandingPage = ({ setView, setShowPaymentModal, isDarkMode }) => {
  return (
    <div className="animate-in fade-in duration-700">
      ...
      <section className="pt-40 pb-20 px-6 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 justify-center md:justify-start">
            <button onClick={() => setView('app')} className="w-full sm:w-auto px-8 py-4 bg-white text-black rounded-full font-bold text-lg shadow-lg hover:scale-105 transition-all flex items-center justify-center gap-2">Start Now <ArrowRight className="w-5 h-5" /></button>
      ...
      <section className="py-20 px-6 max-w-6xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            ...
            <div className="relative p-8 rounded-3xl border border-yellow-500/30 bg-gradient-to-b from-yellow-500/10 to-transparent flex flex-col shadow-2xl"><div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-yellow-500 to-amber-500 text-white px-4 py-1 rounded-full text-xs font-bold shadow-lg">BEST VALUE</div><div className="flex justify-between items-center mb-2"><h3 className="text-xl font-bold text-yellow-500 flex items-center gap-2"><Crown className="w-5 h-5 fill-current"/> God Mode</h3></div><div className="text-4xl font-extrabold mb-6">$5 <span className="text-sm font-normal opacity-60">/ lifetime</span></div><ul className="space-y-4 mb-8 flex-1"><li className="flex items-center gap-2 text-sm"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> <strong>Bulk Compression</strong> (Unlimited)</li><li className="flex items-center gap-2 text-sm"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Priority Support</li><li className="flex items-center gap-2 text-sm"><CheckCircle2 className="w-4 h-4 text-yellow-500"/> Early Access</li></ul><button onClick={() => setShowPaymentModal(true)} className="w-full py-3 rounded-xl font-bold bg-gradient-to-r from-yellow-500 to-amber-600 text-white shadow-lg shadow-yellow-500/20 hover:scale-105 transition-all">Unlock God Mode</button></div>
         </div>
      </section>
